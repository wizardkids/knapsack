...
Usage: knapsack.py [OPTIONS] [MESSAGE]
  ...
  specific recipient, meaning that the sender must have the
  recipient's public key. The encrypted text can only be
  decrypted by the recipient using the recipient's private key.
  Keys can be generated by this program using the --generate option.
